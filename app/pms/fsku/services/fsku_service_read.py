# app/pms/fsku/services/fsku_service_read.py
...
from app.pms.fsku.contracts.fsku import FskuDetailOut, FskuListItem, FskuListOut
from app.pms.fsku.models.fsku import Fsku, FskuComponent
from app.pms.fsku.services.fsku_service_mapper import to_detail
    ...
    rows = (
        db.execute(
            text(
                ...
                FROM pms_fskus f
                LEFT JOIN pms_fsku_components c ON c.fsku_id = f.id
                """
                + where_sql
                + """
                GROUP BY f.id
