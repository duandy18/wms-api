...
class Fsku(Base):
    ...
    __table_args__ = (
        sa.CheckConstraint("shape in ('single', 'bundle')", name="ck_pms_fskus_shape"),
        sa.CheckConstraint("status in ('draft', 'published', 'retired')", name="ck_pms_fskus_status"),
        sa.CheckConstraint("expr_type in ('DIRECT', 'SEGMENT_GROUP')", name="ck_pms_fskus_expr_type"),
        sa.CheckConstraint("component_count >= 0", name="ck_pms_fskus_component_count_nonnegative"),
        sa.Index("ix_pms_fskus_status", "status"),
        sa.Index("ux_pms_fskus_code", "code", unique=True),
        sa.Index("ix_pms_fskus_normalized_expr", "normalized_expr"),
    )
...
class FskuComponent(Base):
    ...
    注意：
    - 该表不是人工主维护真相；
    - 真相来自 pms_fskus.normalized_expr；
    - 本表是表达式展开后的 SKU 组件缓存 / 执行索引。
    """

    __tablename__ = "pms_fsku_components"
    ...
    __table_args__ = (
        sa.CheckConstraint("qty_per_fsku > 0", name="ck_pms_fsku_components_qty_positive"),
        sa.CheckConstraint(
            "alloc_unit_price > 0",
            name="ck_pms_fsku_components_alloc_unit_price_positive",
        ),
        sa.ForeignKeyConstraint(
            ["fsku_id"],
            ["pms_fskus.id"],
            name="fk_pms_fsku_components_fsku",
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["resolved_item_id"],
            ["items.id"],
            name="fk_pms_fsku_components_resolved_item",
            ondelete="RESTRICT",
        ),
        sa.ForeignKeyConstraint(
            ["resolved_item_sku_code_id", "resolved_item_id"],
            ["item_sku_codes.id", "item_sku_codes.item_id"],
            name="fk_pms_fsku_components_resolved_sku_code",
            ondelete="RESTRICT",
        ),
        sa.ForeignKeyConstraint(
            ["resolved_item_uom_id", "resolved_item_id"],
            ["item_uoms.id", "item_uoms.item_id"],
            name="fk_pms_fsku_components_resolved_uom",
            ondelete="RESTRICT",
        ),
        sa.UniqueConstraint(
            "fsku_id",
            "component_sku_code",
            name="uq_pms_fsku_components_fsku_component_sku",
        ),
        sa.UniqueConstraint(
            "fsku_id",
            "sort_order",
            name="uq_pms_fsku_components_fsku_sort",
        ),
        sa.Index("ix_pms_fsku_components_fsku_id", "fsku_id"),
        sa.Index("ix_pms_fsku_components_resolved_item_id", "resolved_item_id"),
        sa.Index("ix_pms_fsku_components_resolved_sku_code_id", "resolved_item_sku_code_id"),
        sa.Index("ix_pms_fsku_components_resolved_uom_id", "resolved_item_uom_id"),
    )
