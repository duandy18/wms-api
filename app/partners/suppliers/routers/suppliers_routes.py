# app/partners/suppliers/routers/suppliers_routes.py
...
from app.partners.suppliers.contracts.suppliers import SupplierCreateIn, SupplierOut, SupplierUpdateIn
from app.partners.suppliers.helpers.suppliers import check_perm, contacts_out
...
def register(router: APIRouter) -> None:
    @router.get("/partners/suppliers", response_model=List[SupplierOut])
    def list_suppliers(
        ...
        check_perm(db, user, ["page.partners.read"])

        suppliers = repo_list_suppliers(db, active=active, q=q)
        return [_to_supplier_out(s) for s in suppliers]

    @router.post("/partners/suppliers", response_model=SupplierOut, status_code=status.HTTP_201_CREATED)
    def create_supplier(
        payload: SupplierCreateIn,
        db: Session = Depends(get_db),
        user=Depends(get_current_user),
    ):
        check_perm(db, user, ["page.partners.write"])
    ...
    @router.patch("/partners/suppliers/{supplier_id}", response_model=SupplierOut)
    def update_supplier(
